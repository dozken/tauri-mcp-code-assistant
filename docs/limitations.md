...
- **A downloaded macOS build will not open on a double-click.**
  The bundles are ad-hoc signed, which makes them well-formed but carries no identity, and
  macOS reports a quarantined app it cannot vouch for as _damaged_ — Sequoia 15.1 removed
  the Control-click → Open escape, so whoever downloads it has to run

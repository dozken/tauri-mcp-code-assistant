...
export const createLangChainTools = (
  ...
  tool(
    async (input: SearchCodeInput) => formatSearchResult(await service.searchCode(input)),
    {
      name: 'search_code',
  ...
  tool(
    async (input: ExplainFileInput) => formatExplainResult(await service.explainFile(input)),
    {
      name: 'explain_file',
  ...
  tool(
    async (input: GenerateSnippetInput) => formatSnippetResult(await service.generateSnippet(input)),
    {
